                    ...
                    <li>
                        <a href="<?= $description_url . '?did=' . $description->id ?>">
                            <div class="cover">
                                <img src="
                                    <?=
                                    (PolicyMS_Public::get_plugin_setting(true, 'marketplace_host')
...
function description_editor_html(PolicyMS_Description $description = null, array $permissions = null): void
{
    ...
    ?>
    <div class="policyms description editor <?= !empty($description) ? 'modalize' : '' ?>">
        <form>
        ...
        foreach ($reviews[0] as $review) { ?>
            <li class="review">
                ...
                <div class="metadata">
                    ...
                    <span>
                        by <a href="<?= PolicyMS_Public::get_plugin_setting(false, 'account_page') . '?user=' . $review->uid ?>"><?= $review->reviewer ?></a>
                        <?php
                        if (!empty($author_id) || $administrator) {
                            if ($review->uid == $author_id || $administrator) {
                                ?>
                                | <button class="action destructive minimal" data-action="delete-review" data-author-id="<?= $review->uid ?>">Delete</button>
                                <?php
                            }
                        }
                        ?>
...
function description_html($description, $image_blobs, $pages, $reviews, $permissions)
{
    ...
    {
        ?>
        <div class="policyms file-viewer <?= ($collapsed) ? 'collapsed' : '' ?>">
                                ...
                                if (!empty($description->assets['videos'])) {
                                    foreach ($description->assets['videos'] as $video) { ?>
                                        <div class="item" data-asset-category="videos" data-asset-id="<?= $video->id ?>">
                                            <img class="play-icon" src="<?= get_site_url(null, '/wp-content/plugins/policyms/public/assets/svg/play.svg'); ?>" />
